function Competences() {
	return (
		<div className='wrapper competences'>
			<h1>Compétences</h1>
			<div className='content'>
				<div className='certifications'>
					<h2>Certifications</h2>
					<div className='group'>
						<h3>TOEIC</h3>
						<span>Date de délivrance: </span>
						<span>Novembre 2022 - Octobre 2024</span>
						<div>910 / 990</div>
					</div>
					<div className='group'>
						<h3>Jr Penetration Tester - Try Hack Me</h3>
						<span>En cours d'apprentissage</span>
						<div>
							Mots clés: Web Hacking, Burp, network security, Vulnerability
							research, metasploit, privilege escalation
						</div>
					</div>
					<div className='group'>
						<h3>Introduction to Cybersecurity - Cisco</h3>
						<span>Date de délivrance: </span>
						<span>Février 2024</span>
						<div>
							Mots clés: Meilleures pratiques en matière de cybersécurité,
							cybersécurité, vulnérabilités des réseaux, vie privée et
							confidentialité des données, détection des menaces.
						</div>
						<button>
							<a
								href='https://www.credly.com/badges/cc6a338d-dfb9-4524-b039-8a9082cdec57/linked_in_profile'
								target='_blank'
							>
								Identifiant
							</a>
						</button>
					</div>
				</div>
				<div className='technical-skills'>
					<h2>Technical Skills</h2>
					<div className='group'>
						<h3>Langages de programmation</h3>
						<div>
							<div>C</div>
							<div>JAVA</div>
							<div>J2EE</div>
							<div>JavaScript</div>
							<div>TypeScript</div>
							<div>Node JS</div>
							<div>PHP</div>
							<div>SQL</div>
						</div>
					</div>
					<div className='group'>
						<h3>Langages de scripting</h3>
						<div>
							<div>Python</div>
							<div>Bash</div>
						</div>
					</div>
					<div className='group'>
						<h3>Réseau</h3>
						<div>
							<div>Modèle OSI</div>
							<div>Modèle IETF</div>
							<div>DHCP</div>
							<div>DNS</div>
							<div>Architecture réseau</div>
							<div>Chaînes de Markov</div>
							<div>Graphes</div>
						</div>
					</div>
					<div className='group'>
						<h3>Outils Réseaux</h3>
						<div>
							<div>Cisco Packet Tracer</div>
							<div>Wireshark</div>
						</div>
					</div>
					<div className='group'>
						<h3>Systèmes d'exploitation</h3>
						<div>
							<div>Windows</div>
							<div>Linux (Debian, Kali, Parrot OS)</div>
							<div>Virtualisation</div>
						</div>
					</div>
					<div className='group'>
						<h3>Frameworks</h3>
						<div>
							<div>React</div>
							<div>Jest</div>
							<div>Express.js</div>
							<div>Spring boot</div>
						</div>
					</div>
					<div className='group'>
						<h3>Outils DevOps</h3>
						<div>
							<div>Docker</div>
							<div>Ansible</div>
							<div>Base de CI/CD</div>
						</div>
					</div>
					<div className='group'>
						<h3>Autres outils</h3>
						<div>
							<div>Git & Github</div>
							<div>Splunk</div>
						</div>
					</div>
					<div className='group'>
						<h3>Bases de données</h3>
						<div>
							<div>MySQL</div>
							<div>MongoDB</div>
							<div>PostreSQL</div>
							<div>Spring boot</div>
						</div>
					</div>
					<div className='group'>
						<h3>Gestion de projets</h3>
						<div>
							<div>Scrum</div>
						</div>
					</div>
				</div>
				<div className='soft-skills'>
					<h2>Soft skills</h2>
					<div className='group'>
						<h3>Communication</h3>
					</div>
					<div className='group'>
						<h3>Esprit d'analyse</h3>
					</div>
					<div className='group'>
						<h3>Rigueur</h3>
					</div>
					<div className='group'>
						<h3>Adaptabilité</h3>
					</div>
					<div className='group'>
						<h3>Facilité d'assimilation</h3>
					</div>
					<div className='group'>
						<h3>Curiosité</h3>
					</div>
					<div className='group'>
						<h3>Travail d'équipe</h3>
					</div>
					<div className='group'>
						<h3>Autonomie</h3>
					</div>
				</div>
			</div>
		</div>
	);
}

export default Competences;
